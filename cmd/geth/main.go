...
// geth is the official command-line client for Sberex.
package main
...
var (
	...
	gitCommit = ""
	// Sberex address of the Geth release oracle.
	relOracle = common.HexToAddress("0xfa7b9770ca4cb04296cac84f37736d4041251cdf")
	// The app that holds all commands and flags.
	app = utils.NewApp(gitCommit, "the go-sberex command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2018 The go-sberex Authors"
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Sberex node is running
		if ctx.GlobalBool(utils.LightModeFlag.Name) || ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		var sberex *eth.Sberex
		if err := stack.Service(&sberex); err != nil {
			utils.Fatalf("Sberex service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := sberex.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		// Set the gas price to the limits from the CLI and start mining
		sberex.TxPool().SetGasPrice(utils.GlobalBig(ctx, utils.GasPriceFlag.Name))
		if err := sberex.StartMining(true); err != nil {
