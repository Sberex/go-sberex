...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.sberex", new NodeConfig());
			node.start();
			...
			SberexClient ec = node.getSberexClient();
	...
	// signatures too. Regression test for go-sberex issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/sberex/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.sberex", "github.com/Sberex/go-sberex/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "sberex", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
