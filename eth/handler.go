...
// NewProtocolManager returns a new sberex sub protocol manager. The Sberex sub protocol manages peers capable
// with the sberex network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Sberex peer", "peer", id)

	// Unregister the peer from the downloader and Sberex peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Sberex protocol")
	...
	log.Info("Sberex protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Sberex peer connected", "name", p.Name())

	// Execute the Sberex handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Sberex handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Sberex peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Sberex message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Sberex sub-protocol metadata
